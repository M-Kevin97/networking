...
@Component({
  selector: 'app-item-tags-form',
  templateUrl: './item-tags-form.component.html',
  styleUrls: ['./item-tags-form.component.scss']
})
export class ItemTagsFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    if(this.itemFormService.isTagsOk())  this.onRestoreCategoriesTag(this.itemFormService.item.tags);
    // else if the earlier elements had not been created, retun to the start
    // else {
    //   this.itemFormService.onStartToTheBeginning();
    // }
    else if(!this.itemFormService.isTitleOk()) {
  ...
  onSetTags() {
    ...
    console.log('onSetTags', tags);
    this.itemFormService.item.tags = tags;

    // this.itemFormService.setFormWithStepState(StepState.CATEGORIES, tags);
